 ...
 *
 * By default dltaskd functions as a tasking daemon, executing and managing
...
#define MAX_ERRORS		32	/* Used to avoid infinite loops */
...
static int connfd[MAX_CONN];		/* Active client connections */
static struct task_t tasks[MAX_TASKS];	/* Active tasks */
static char *taskName;			/* Name of task being processed */
static char *dirs[MAX_DIRS];		/* Task search path */
static int ndir;			/* Number of directories */
static int nconn;			/* Number of active connections */
static int ntask;			/* Number of active tasks */
 ...
 * be the final dltaskd argument, and any following arguments are passed to
 * the executed task.
 */
int
main (int argc, char *argv[])
{
    ...
    static char keyopts[] = "cd:h:p:st:v";
	...
	if (result > 0) {
	    ...
	    fprintf (stderr, "dltaskd: select() error: %s\n", strerror(errno));
	    if (nerror++ > MAX_ERRORS) {
		fprintf (stderr, "dltaskd: too many select() errors\n");
		close (listenfd);
		exit (10);
	    }
	}
...
handleRequest (int conn)
{
    ...
    *op = '\0';
	...
	if (tp->pid != 0) {
	    ...
	    if (verbose)
		fprintf (stderr, "execute task: conn=%d, task='%s %s'\n",
		    conn, taskPath, tp->taskArgs);
...
sigchld_handler (int sig)
{
	...
	if (tp) {
	    ...
	    if (verbose)
		fprintf (stderr, "task exited: conn=%d, task=%s, stat=%d\n",
