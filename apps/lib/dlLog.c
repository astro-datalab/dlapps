...
/**
 *  DL_APPLOG -- VOApp message logger.
 *  
 *  @fn	     dl_appLog (char *format, ...)
 *
...
dl_appLog (FILE *fd, char *format, ...)
{
    ...
    dl_encodeString (buf, format, &argp);
    va_end (argp);
...
/**
 *  DL_ENCODESTRING -- Process the format to the output file, taking arguments
 *  from the list pointed to by argp as % format specs are encountered in 
 *  the input.
 *
 *  @fn	     dl_encodeString (char *buf, char *format, va_list *argp)
 *
 ...
 */
void
dl_encodeString (char *buf, char *format, va_list *argp)
{
      ...
      if (ch == '%') {
    	...
    	*fsp++ = ch;
    	...
    	while (!done) {
    	    ...
    	    case 'u':
    		*fsp = EOS;
    		strcat (buf, dl_doarg (&argp, TY_INT));
    		done++;
    		break;
...
rval:
    		*fsp = EOS;
    		strcat (buf, dl_doarg (&argp, TY_DOUBLE));
    		done++;
    		break;

    	    case 's':
    		*fsp = EOS;
    		strcat (buf, dl_doarg (&argp, TY_CHAR));
    		done++;
    		break;
    	    }
    	}
...
/**
 *  DL_DOARG -- Encode a single argument acording to the data type.
 *
 *  @fn     static char *dl_doarg (va_list **argp, int dtype)
 *
 ...
 */
char * 
dl_doarg (va_list **argp, int dtype)
{
    int		ival;
    double	dval;
    char	*cptr;
