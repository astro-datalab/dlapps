...
**    #include <ctype.h>
**
**    int dl_decodeRanges(char *str, int *ranges, int max_ranges,int *nvalues);
...
**        for (i=0; rtest[i]; i++) {
** 	    printf ("dl_decodeRanges: '%s'\n\n   ", rtest[i]);
**	    val = dl_decodeRanges (rtest[i], ranges, max_ranges, &nvalues);
...
*/
int 
dl_decodeRanges (range_string, ranges, max_ranges, nvalues)
