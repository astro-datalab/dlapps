 ...
 *  Private methods.
 */
static void  dl_printFrameInfo (FILE *fd, frameInfo *im);
static  int  dl_getFrameWcs (fitsfile *fptr, frameInfo *info);

extern  int  dl_fileType (char *name);




/**
 *  DL_IMAGEINFO -- Get information about a FITS file structure and WCS.
 *  
 *  @fn      info = dl_imageInfo (char *name, int do_all)
 *
 ...
 */
ImInfo *
dl_imageInfo (char *name, int do_all)
{
    fitsfile *fptr;
	...
	info->extns = (frameInfo *) calloc (nextns, sizeof (frameInfo));
		...
		if (hdupos == 0) {				/* PHU	*/
		    ...
		    if (dl_getFrameWcs (fptr, &info->extns[extnum]) == 0)
...
/**
 *  DL_IMAGENEXTNS -- Get the number of extensions in an MEF file.
 *
 *  @fn      nextn = dl_imageNExtns (char *name)
 *
...
/**
 *  DL_PRINTIMAGEINFO -- Print the image information.
 *
 *  @fn      dl_printImageInfo (FILE *fd, ImInfo *im)
 *
 ...
 */
void
dl_printImageInfo (FILE *fd, ImInfo *im)
{
    register int i;
    ...
    fprintf (fd, "Frame:\n");   im->frame.extnum = -1;
        dl_printFrameInfo (fd, &im->frame);

    for (i=1; i < im->nextend; i++)
	dl_printFrameInfo (fd, &im->extns[i]);
}
...
/**
 *  DL_PRINTFRAMEINFO -- Print information about a specific frame.
 */
static void
dl_printFrameInfo (FILE *fd, frameInfo *im)
{
    if (im->extnum >= 0) {
        fprintf (fd, "\nExt: %d  dims[%d] = %d %d %d\t",
...
/**
 *  DL_GETFRAMEWCS -- Get the WCS information for a given frame.
 */
static int  
dl_getFrameWcs (fitsfile *fptr, frameInfo *info)
{
...
main (int argc, char *argv[])
{
    ...
    if ( (img = dl_imageInfo (argv[1], 1)) ) {
	dl_printImageInfo (stdout, img);
	dl_freeImageInfo (img);
    }
