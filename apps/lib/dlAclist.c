...
extern  char *dl_validateFile (char *fname);
extern  char *dl_urlFname (char *url);
extern  char *dl_getURL (char *url, char *fname);


/* Local task prototypes.
*/
void    dl_addToAclist (char *url, char *fname);
void    dl_freeAclist (void);
void    dl_procAclist (void);

static pid_t dl_dlProc (Acref *ac, int filenum);
static int   dl_acGetURL (char *url, char *fname, long *size);



/************************************************************************
**  DL_ADDTOACLIST --  Add the given url/id to the file access list.
*/
void
dl_addToAclist (char *url, char *outfile)
{
...
/************************************************************************
**  DL_FREEACLIST --  Free the Aclist structure and reset the counter.
*/
void
dl_freeAclist ()
{
...
/************************************************************************
**  DL_PROCACLIST -- Begin processing the access list.  We do this by
...
/************************************************************************
**  DL_DLPROC -- Procedure used to spawn a download child process.  We
**  return the child pid, and fork off the actual download.  The caller
**  will wait for completion. 
*/
static pid_t
dl_dlProc (Acref *ac, int filenum)
{
    pid_t  cpid;
    int    nerrs = 0;
    long   nbytes = 0;
    ...
    if (debug)
	fprintf (stderr, "dl_dlProc(%d): %s\n", filenum, ac->url);

    if ((cpid = fork()) < 0) {
	fprintf (stderr,
	    "dl_dlProc: Unable to create child process, exiting\n");
	exit (-1);
	...
	urlFname = dl_urlFname(ac->url);
        ...
        nerrs = dl_acGetURL (ac->url, fname, &nbytes);

        out = dl_validateFile (fname);
...
static int
dl_acGetURL (char *url, char *fname, long *size)
{
#ifdef FOO
    int  fd, nbytes, err = 0;
    char *res;
...
#ifdef FOO
    if ((res = dl_getURL (url, &nbytes)) == NULL) {
