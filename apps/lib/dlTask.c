...
/*  Internal procedures.
 */
static pid_t 	dl_taskExec (char *method, Task *apps, int argc, char *argv[]);
static int	dl_taskWait (pid_t pid, size_t *len);
static void    *dl_taskResult (pid_t pid, size_t len);
static void     dl_taskReaper (int sig, int *arg1, int *arg2);
static size_t   dl_taskRead (int fd, void *data, size_t len);
static size_t   dl_taskWrite (int fd, void *data, size_t len);
...
/**
 *  DL_RUNTASK -- Run a VOApps task as a connected subprocess.
 *
 *  @brief  Run a VOApps task as a connected subprocess.
 *  @fn     int dl_runTask (char *method, Task *apps, int argc, char *argv[], 
...
static int	dl_signal_status = 0;
static int	dl_exit_status   = 0;
sigjmp_buf 	dl_env;

int
dl_runTask (char *method, Task *apps, int argc, char **argv, size_t *len, void **result)
{
    pid_t  pid;
    ...
    old_sigcld = (SIGFUNC) signal (SIGCHLD, (SIGFUNC) dl_taskReaper);
    dl_signal_status = 0;
    dl_exit_status   = 0;

    sigsetjmp (dl_env, 1);
    if (dl_signal_status || dl_exit_status) {
	if (dl_signal_status) 
	    sprintf (err, "Child exited with SIGNAL %d", dl_signal_status);
	else if (dl_exit_status > 1) 
	    sprintf (err, "Child exited with status %d", dl_exit_status);
	*len    = strlen (err);
	*result = (void *) strdup (err);
    ...
    if ((pid = dl_taskExec (method, apps, argc, argv)) > 0) {

        if ((status = dl_taskWait (pid, &resLen)) == EXIT_SUCCESS) {

            if (resLen && (res = dl_taskResult (pid, resLen)))
...
/**
 *  DL_TASKTEST -- Execute a task as a unit test.
 *
 *  @brief  Execute a task as a unit test.
 *  @fn     int dl_taskTest (Task *self, char *arg, ...)
 *
 ...
 */
int  
dl_taskTest (Task *self, char *arg, ...)
{
    ...
    if (!getenv ("VOAPP_CONNECTED"))
        status = dl_runTask (self->name, self, argc, argv, &reslen, &result);
    else
...
/**
 *  DL_TASKDBG -- Tasking debug breakpoint.
 *
 *  @brief  Tasking debug breakpoint.
 *  @fn     void dl_taskDbg (void)
 *
 *  @return             nothing
 */
...
/**
 *  DL_TASKTESTREPORT -- Report from the task testing interface.
 *
 *  @brief  Report from the task testing interface.
 *  @fn     void dl_taskTestReport (self)
 *
 *  @param   self	task struct pointer
 *  @return             status (0=OK, 1=ERR)
 */
void
dl_taskTestReport (Task self)
{
    fprintf (stderr, "\n");
    fprintf (stderr, 
...
/**
 *  DL_TASKTESTFILE -- Create a small named test file.
 *
 *  @brief  Create a small named test file.
 *  @fn     void dl_taskTestFile (char *str, char *fname)
 *
 ...
 */
void
dl_taskTestFile (char *str, char *fname)
{
    FILE *fd = (FILE *) NULL;
...
/**
 *  DL_SETRESULTFROMFILE -- Set a result object to a file's contents.
 *
 *  @brief  Set a result object to a file's contents.
 *  @fn     int dl_setResultFromFile (char *fname, size_t *len, void **data)
 *
 ...
 */
int
dl_setResultFromFile (char *fname, size_t *len, void **data)
{
    struct stat  st;
    int  fd = 0;
	...
	*data = calloc (1, st.st_size + 1);
	if (dl_taskRead (fd, *data, *len) < *len)
	    return (ERR);
	close (fd);
    } else
	return (ERR);
...
/**
 *  DL_SETRESULTFROMINT -- Set a result object to an int value.
 *
 *  @brief  Set a result object to an int value.
 *  @fn     int dl_setResultFromInt (int value, size_t *len, void **data)
 *
 ...
 */
int
dl_setResultFromInt (int value, size_t *len, void **data)
{
    char  buf[SZ_FNAME];
...
/**
 *  DL_SETRESULTFROMREAL -- Set a result object to a real value.
 *
 *  @brief  Set a result object to a real value.
 *  @fn     int dl_setResultFromReal (float value, size_t *len, void **data)
 *
 *  @param   value      value to set
 *  @param   len        length of result object
 *  @param   data       pointer to result object
 ...
 */
int
dl_setResultFromReal (float value, size_t *len, void **data)
{
    char  buf[SZ_FNAME];
...
/**
 *  DL_SETRESULTFROMSTRING -- Set a result object to a string value.
 *
 *  @brief  Set a result object to a string value.
 *  @fn     int dl_setResultFromString (char *str, size_t *len, void **data)
 *
 *  @param   value      value to set
 *  @param   len        length of result object
 *  @param   data       pointer to result object
 ...
 */
int
dl_setResultFromString (char *str, size_t *len, void **data)
{
    if (str) {
        *len = strlen (str);
        *data = (char *) calloc (1, *len + 1);
...
/**
 *  DL_APPENDRESULTFROMSTRING -- Append a result object to a string value.
 *
 *  @brief  Append a result object to a string value.
 *  @fn     int dl_appendResultFromString (char *str, size_t *len, 
 ...
 */
int
dl_appendResultFromString (char *str, size_t *len, void **data, size_t *maxlen)
{
    int  slen = strlen (str);
...
)
{
    ...
    dl_signal_status = dl_exit_status  = 0;

    while ((pid = (int) waitpid ((pid_t) 0, &status, WNOHANG)) > 0)
	if (status) {
	    if (status & 0xFF)
		dl_signal_status = (status & 0xFF);
	    else if (status >> 8)
		dl_exit_status = (status >> 8);
	    siglongjmp (dl_env, 1);
	}
}
...
/**
 *  DL_TASKEXEC -- Execute a task as a connected subprocess.
 *
 *  @brief  Get the result object (if any) from the task.
 *  @fn     Task *dl_taskExec (char *method, Task *apps, int argc, char *argv[])
 *
 *  @param   method     task name to call
...
static pid_t
dl_taskExec (char *method, Task *apps, int argc, char **argv)
{
    Task  *app = (Task *) NULL;
    int	   status = 0;
    pid_t  cpid;
	...
	do {
	    memset (msg, 0, 128);
            nr = dl_taskRead (PARENT_READ, msg, GO_LEN);
	...
	for (app = apps; app->name; app++) {
		 ...
		 */
        	dl_taskWrite (PARENT_WRITE, &status, sizeof (int));
        	dl_taskWrite (PARENT_WRITE, &resLen, sizeof (size_t));
		if (resLen)
            	    dl_taskWrite (PARENT_WRITE, result, resLen);
		break;
	    }
	}
	...
	if (app->name == NULL) {
	    status = -1;
            dl_taskWrite (PARENT_WRITE, &status, sizeof (int));
            dl_taskWrite (PARENT_WRITE, E_NOTFOUND, strlen (E_NOTFOUND));
	}
	 ...
	 */
        ...
        dl_taskWrite (CHILD_WRITE, GO_MSG, GO_LEN);
...
/**
 *  DL_TASKWAIT -- Wait for a child process to complete.
 *
 *  @brief  Wait for a child process to complete.
 *  @fn     int dl_taskWait (pid_t pid, int *len)
 *
 ...
 */
static int
dl_taskWait (pid_t pid, size_t *len)
{
    int   status = 0;
     ...
     */
    dl_taskRead (CHILD_READ, &status, sizeof (int));
    dl_taskRead (CHILD_READ, len, sizeof (size_t));
...
/**
 *  DL_TASKRESULT -- Get the result object (if any) from the task.
 *
 *  @brief  Get the result object (if any) from the task.
 *  @fn     void *dl_taskResult (pid_t pid, size_t len)
 *
 *  @return             (allocated) pointer to the Task
 */
static void *
dl_taskResult (pid_t pid, size_t len)
{
    void   *result = (void *) calloc (1, len);

    dl_taskRead (CHILD_READ, result, len);
...
/**
 *  DL_TASKREAD -- Read exactly "n" bytes from a task descriptor. 
 *
 *  @brief  Read exactly "n" bytes from a task descriptor. 
 *  @fn     size_t dl_taskRead (int fd, void *vptr, size_t nbytes)
 *
 ...
 */
static size_t  
dl_taskRead (int fd, void *vptr, size_t nbytes)
{
    char    *ptr = vptr;
...
/**
 *  DL_TASKWRITE -- Write exactly "n" bytes to a task descriptor. 
 *
 *  @brief  Send exactly "n" bytes to a task descriptor. 
 *  @fn     size_t dl_taskWrite (int fd, void *vptr, size_t nbytes)
 *
 *  @param  fd          file descriptor
 *  @param  vptr        data buffer to be written
 *  @param  nbytes      number of bytes to write
 ...
 */
static size_t  
dl_taskWrite (int fd, void *vptr, size_t nbytes)
{
    char    *ptr = vptr;
...
int runTest (char *task, int argc, char **argv)
{
    ...
    status = dl_runTask (task, argc, argv, &len, &res);
